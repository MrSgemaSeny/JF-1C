...
public interface TaskRepository extends JpaRepository<Task, Long>, JpaSpecificationExecutor<Task> {
    ...
    @Query("SELECT SUM(t.amount) FROM Task t WHERE t.stage.type = 'WON'")
    java.math.BigDecimal sumWonAmount();

    @Query("SELECT SUM(t.amount) FROM Task t WHERE t.stage.type != 'LOST'")
    ...
    @Query("select t from Task t join fetch t.client c left join fetch t.assignedTo left join fetch t.createdBy left join fetch t.stage s " +
           "where t.dueDate = :dueDate and s.type not in :excludedTypes and t.deadlineNotifiedAt is null")
    ...
    @Query("SELECT new com.example.zhanfinancebackend.modules.crm.dto.ClientStatsDto(t.client.id, COUNT(t.id)) FROM Task t GROUP BY t.client.id")
    List<com.example.zhanfinancebackend.modules.crm.dto.ClientStatsDto> getClientStats();

    @Query("SELECT s.name as statusName, COUNT(t.id) as count FROM Task t LEFT JOIN t.stage s GROUP BY s.name")
    List<java.util.Map<String, Object>> countTasksByStatus();

    @Query("SELECT COUNT(t.id) FROM Task t WHERE t.stage.type = :type")
    ...
    @Query("SELECT COALESCE(t.lostReason, 'Не указана') as reason, COUNT(t.id) as count FROM Task t WHERE t.stage.type = 'LOST' GROUP BY COALESCE(t.lostReason, 'Не указана')")
    List<java.util.Map<String, Object>> countTasksByLostReason();

    @Query(value = "SELECT COALESCE(t.assigned_to_id, c.assigned_employee_id) as empId, s.type as stageType, COUNT(t.id) as taskCount, SUM(CASE WHEN t.due_date < CURRENT_DATE AND (s.type IS NULL OR s.type NOT IN ('WON', 'LOST')) THEN 1 ELSE 0 END) as overdueCount FROM tasks t LEFT JOIN app_users c ON t.client_id = c.id LEFT JOIN stages s ON t.stage_id = s.id WHERE COALESCE(t.assigned_to_id, c.assigned_employee_id) IS NOT NULL GROUP BY COALESCE(t.assigned_to_id, c.assigned_employee_id), s.type", nativeQuery = true)
    List<java.util.Map<String, Object>> getEmployeeTaskStats();

    @Query("SELECT s.name as statusName, COUNT(t.id) as count FROM Task t LEFT JOIN t.stage s WHERE t.client.id = :clientId GROUP BY s.name")
    List<java.util.Map<String, Object>> countTasksByStatusForClient(@Param("clientId") Long clientId);

    @Query("SELECT COUNT(t.id) FROM Task t LEFT JOIN t.client c WHERE t.assignedTo = :employee OR (t.assignedTo IS NULL AND c.assignedEmployee = :employee)")
    long countTasksForEmployee(@Param("employee") User employee);

    @Query("SELECT s.name as statusName, COUNT(t.id) as count FROM Task t LEFT JOIN t.stage s LEFT JOIN t.client c WHERE t.assignedTo = :employee OR (t.assignedTo IS NULL AND c.assignedEmployee = :employee) GROUP BY s.name")
