...
public interface CourseRepository extends JpaRepository<Course, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("select c from Course c where c.isPublished = true and (lower(c.title) like lower(concat('%', :query, '%')) or lower(c.description) like lower(concat('%', :query, '%')))")
