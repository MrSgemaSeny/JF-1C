...
export function ContactForm({ title = 'Связаться с нами', className = '', showMessage = false }: ContactFormProps) {
  const { name, setName, phone, setPhone, message, setMessage, submitted, loading, error, handleSubmit } = useContactForm();

  return (
    <div className={className}>
      {title && <h3 className="text-3xl font-black uppercase mb-8">{title}</h3>}
      {submitted ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {showMessage && (
            <div>
              <label className="block text-sm font-bold mb-2 uppercase tracking-wider opacity-70">Краткое описание задачи</label>
              <textarea
                value={message}
