...
const teamRows = [
  ...
  [
    {
      name: 'Имя Фамилия',
      role: 'Первичная документация',
      photo: '',
    },
    {
      name: 'Имя Фамилия',
      role: 'Сверки и отчётность',
      photo: '',
    },
    {
      name: 'Имя Фамилия',
    ...
    {
      name: 'Имя Фамилия',
      role: 'Восстановление учета',
      photo: '',
    },
    {
      name: 'Имя Фамилия',
      role: 'Проверка контрагентов',
      photo: '',
    },
  ],
];
...
function TeamCard({
  ...
  member: (typeof teamRows)[number][number];
  delay: number;
  isLead?: boolean;
  isGreen?: boolean;
}) {
            ...
            {member.photo ? (
              ...
              <div className={[
                ...
                <div className={[
                  ...
                  useGreenTheme ? 'border-brand-beige/20 bg-brand-beige/10' : 'border-brand-green/15 bg-brand-beige/70',
                ].join(' ')}>
                  Фото
                </div>
                <p className="text-sm uppercase tracking-[0.28em] font-bold opacity-70">Шаблон под фотографию 3:4</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {teamRows[1].map((member, index) => (
            <TeamCard key={member.name} member={member} delay={0.1 + index * 0.08} />
          ))}
        </div>

        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-5">
          {teamRows[2].map((member, index) => (
            <TeamCard key={member.name} member={member} delay={0.2 + index * 0.08} isGreen />
          ))}
        </div>
      </div>
    </Section>
