                ...
                {invoices.map((invoice) => (
                  <tr key={invoice.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-3 text-sm">
                      {invoice.status === 'PAID' ? (
                        <span className="px-2.5 py-1 bg-green-100 text-green-800 rounded-md text-xs font-semibold">{t('adminInvoices.statusPaid')}</span>
                      ) : invoice.status === 'OVERDUE' ? (
                        <span className="px-2.5 py-1 bg-red-100 text-red-800 rounded-md text-xs font-semibold">{t('adminInvoices.statusOverdue')}</span>
                      ) : (
                        <span className="px-2.5 py-1 bg-yellow-100 text-yellow-800 rounded-md text-xs font-semibold">{t('adminInvoices.statusIssued')}</span>
                      )}
                    </td>
            ...
            <form onSubmit={handleCreateInvoice} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button type="submit" disabled={isSubmitting}>
                  {isSubmitting ? <Spinner size="sm" /> : t('adminInvoices.issueBtn')}
                </Button>
              </div>
            </form>
          </div>
