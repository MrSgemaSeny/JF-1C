...
export function ClientDocumentsPage() {
  ...
  return (
    <div className="w-full max-w-[1400px] px-4 md:px-8 mx-auto space-y-6 pb-24 pt-4">
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-3.5 rounded-2xl shadow-2xl z-40 flex items-center gap-5 border border-gray-800 animate-in slide-in-from-bottom-5 duration-200">
          <span className="text-xs font-bold text-gray-300">
            Выбрано документов: <strong className="text-white font-extrabold">{selectedIds.size}</strong>
          </span>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-1.5 bg-brand-green hover:bg-brand-green/90 text-white rounded-xl font-bold text-xs transition-all shadow-xs cursor-pointer disabled:opacity-50"
          >
            <FileArchive size={15} />
            <span>
              {isDownloadingZip
