                    ...
                    <div className="pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => handleDownload(doc)}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-50 text-gray-700 hover:text-brand-green hover:bg-emerald-50 border border-gray-200/80 hover:border-brand-green/30 rounded-lg font-bold text-xs transition-all cursor-pointer"
                          title={t('documents.actions.download', { defaultValue: 'Скачать' })}
                        >
                          <Download size={14} />
                          <span>{t('documents.actions.download', { defaultValue: 'Скачать' })}</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => handleDelete(doc.id)}
                          className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 border border-transparent hover:border-red-200/50 rounded-lg transition-all cursor-pointer"
                          title={t('documents.actions.delete', { defaultValue: 'Удалить' })}
                        >
                          <Trash2 size={16} />
                        </button>
