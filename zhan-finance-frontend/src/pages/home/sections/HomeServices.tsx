        ...
        {isLoading ? (
          ...
          const renderCard = (service: ServiceDto, i: number, isOutsourceCol: boolean) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-brand-beige/20 p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[28px] border border-brand-green/10 hover:border-brand-green/30 hover:bg-brand-beige transition-all group flex flex-col cursor-pointer justify-between shadow-sm hover:shadow-md"
              onClick={() => setSelectedService(service)}
            >
              <div>
                <h3 className="text-base sm:text-xl lg:text-2xl font-black uppercase text-brand-green mb-1.5 sm:mb-2 leading-tight">
                  {t(`service.${service.id}.title`, { defaultValue: service.title })}
                </h3>
                <p className="text-xs sm:text-sm text-brand-green/75 mb-3 sm:mb-4 leading-relaxed line-clamp-2 sm:line-clamp-3">
                ...
                <ul className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-5">
                  {service.features.slice(0, 3).map((feature, featureIndex) => (
                    <li key={feature} className="flex items-start gap-2 text-xs font-bold text-brand-green/80">
              ...
              <div className="pt-3 sm:pt-4 border-t border-brand-green/10 flex items-center justify-between gap-2 mt-auto">
                <span className="text-brand-green font-black uppercase tracking-wider text-xs inline-flex items-center gap-1.5 group-hover:gap-2.5 transition-all">
                  {t('homeServices.more', { defaultValue: 'Подробнее' })} <ArrowRight className="w-3.5 h-3.5" />
                </span>
                {isOutsourceCol && (
                  <span className="text-xs font-black text-brand-green bg-brand-green/10 px-2.5 py-0.5 sm:py-1 rounded-full whitespace-nowrap">
