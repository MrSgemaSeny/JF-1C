...
import { HandwrittenArrow, HandwrittenText } from '@/shared/ui/Handwritten';
        ...
        <div className="space-y-8 order-2 lg:order-1">
          ...
          >
            <p className="text-xl text-brand-green/80 leading-relaxed font-medium">
              Мы сами проходим проверки, тестируем схемы налогообложения и продумываем каждую деталь в учете — чтобы вам оставалось только масштабировать компанию.
            </p>
            
            <button className="px-8 py-4 bg-brand-green text-brand-beige rounded-2xl font-bold uppercase tracking-wider hover:bg-brand-green/90 transition-all hover:-translate-y-1 shadow-lg shadow-brand-green/20">
              Подробнее о нас
            </button>
          </motion.div>
        </div>
          ...
          {polaroids.map((p, i) => (
            ...
            >
              <div className="w-full h-[280px] bg-brand-green/10 overflow-hidden relative group">
                <img src={p.src} alt={p.alt} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700 ease-out" />
              </div>
            </motion.div>
          ))}
