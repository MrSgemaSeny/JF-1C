...
- `[WARNING]` **React Router & AuthContext Unmounting**: При отправке формы логина/регистрации (или логина через Google) нельзя выставлять глобальный флаг `isLoading = true` в `AuthContext`, если он скрывает `RouterProvider`. Это приведет к полному unmount'у приложения, потере `location.state` (важно для передачи `preAuthToken` при 2FA) и локальных стейтов компонентов (например, текущего шага `CREDENTIALS`/`TOTP` в `LoginPage`). Флаг загрузки контекста должен использоваться только для начального `/auth/me` запроса.
