...
| US-01.5  | *Любая*   | безопасно восстанавливать пароль по email | вернуть доступ к аккаунту без перечисления пользователей | Done   |
...
- **Безопасное восстановление пароля (#25)** — криптографический SHA-256 токен (TTL 15 минут), анти-перечисление (Zero-Enumeration), строгий rate limit (3 req / 15 min), автоматический отзыв всех refresh-токенов при сбросе (ADR-016).
...
- [x] [US-01.5] Сброс пароля отправляет одноразовый SHA-256 токен на email, инвалидирует старые токены и сессии.
