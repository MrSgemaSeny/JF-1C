...
- **InvoiceAccessService** — централизованная проверка прав доступа к инвойсам, исключающая возможность просмотра чужих счетов клиентами (IDOR защита). Клиенты (`CLIENT`) имеют права только на чтение (`canRead`) своих счетов; создание и редактирование (`canWrite`, `canCreateFor`) строго ограничено ролями `ADMIN` и `EMPLOYEE`.
